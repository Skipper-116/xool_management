...
module Api
  module V1
    class StudentRegistriesController < ApplicationController
      before_action :set_student_registry, only: %i[show update destroy]
      ...
      def student_registry_params
        params.require(:student_registry).permit(:classroom_id, :cohort_term_id, :person_id, :captured_by, :present,
                                                 :absent_reason)
      end
    end
  end
